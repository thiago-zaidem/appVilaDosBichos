...
Template.afFileUpload.events
	...
	'click .file-dropped': (event, template)->
		template.$('.file-upload').click()
		return
...
Template.afFileUpload.helpers
	collection: ->
		getCollection(@)
	label: ->
		@atts.label or 'Escolha um arquivo'
	buttonEnabled: ->
		@atts.buttonEnabled
	buttonlabel: ->
		@atts.buttonlabel or 'Escolha um arquivo'
	dropLabel: ->
		@atts.dropLabel or 'Clique ou arraste e solte os arquivos aqui'
	dropEnabled: ->
		@atts.dropEnabled
	dropClasses: ->
		@atts.dropClasses or 'card-panel grey lighten-4 grey-text text-darken-4'
	removeLabel: ->
		@atts['remove-label'] or 'Remover'
	fileUploadAtts: ->
