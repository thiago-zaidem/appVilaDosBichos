  ...
  if checkbox
    checkboxField =
      ...
      fn: (value, object, key) ->
        ...
        checked = ''
        html = '<input type="checkbox"'+ checked + ' name="tbItem" value="'+value+'" />'
...
# Prevent the check from being considered as a click event on the row.
  ...
  'change #checkAll': (e, template) ->
    selectAll = $('#checkAll').prop('checked')
    if selectAll
      template.$('[name="tbItem"]').prop('checked',true)
    else
      template.$('[name="tbItem"]').prop('checked',false)
