Meteor.methods
  adminInsertDoc: (doc,collection)->
    ...
    if (Roles.userIsInRole this.userId, ['admin']) or (Roles.userIsInRole this.userId, Config.profileDashboard)
      if checkUserAction "Inserir", this.userId
        this.unblock()
  ...
  adminUpdateDoc: (modifier,collection,_id)->
    check arguments, [Match.Any]
    userAction = ((checkUserAction "Editar", this.userId) or (checkUserAction "Editar_um", this.userId))
    if (Roles.userIsInRole this.userId, ['admin']) or (Roles.userIsInRole this.userId, Config.profileDashboard)
      if userAction
        this.unblock()
  ...
  adminRemoveDoc: (collection,_id)->
    check arguments, [Match.Any]
    if (Roles.userIsInRole this.userId, ['admin']) or (Roles.userIsInRole this.userId, Config.profileDashboard)
      if checkUserAction "Apagar", this.userId
        if collection == 'Users'
  ...
  adminNewUser: (doc) ->
    check arguments, [Match.Any]
    checkUserAction "Inserir", this.userId
    if Roles.userIsInRole this.userId, ['admin']
      ...
      _.each emails, (email)->
        ...
        #console.log(doc.accessProfile)
        user.roles = doc.accessProfile
        Roles.addUsersToRoles _id, doc.accessProfile

  adminUpdateUser: (modifier,_id)->
    check arguments, [Match.Any]
    userAction = ((checkUserAction "Editar", this.userId) or (checkUserAction "Editar_um", this.userId))
    if userAction or Roles.userIsInRole this.userId, ['admin']
    # if Roles.userIsInRole this.userId, ['admin']
      this.unblock()
