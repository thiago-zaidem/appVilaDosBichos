...
Template.PanelLayout.events
	'click .btn-delete': (e,t) ->
		_id = $(e.target).attr('doc')
		Session.set 'panel_id', parseID(_id)
		Session.set 'panel_doc', adminCollectionObject(Session.get('panel_collection_name')).findOne(parseID(_id))

Template.PanelDeleteModal.events
	'click #confirm-delete': () ->
		collection = Session.get 'panel_collection_name'
		_id = Session.get 'panel_id'
		Meteor.call 'adminRemoveDoc', collection, _id, (e,r)->
			$('#panel-delete-modal').modal('hide')